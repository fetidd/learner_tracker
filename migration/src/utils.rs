...
pub fn generate_pupil(rng: &mut ThreadRng) -> Pupil {
    ...
    let mat = [(true, 1), (false, 10)]
        .choose_weighted(rng, |ch| ch.1)
        .unwrap()
        .0;
    let aln = if mat {
        false
    ...
    Pupil {
        ...
        active: Set([(true, 15), (false, 1)]
            .choose_weighted(rng, |ch| ch.1)
            .unwrap()
            .0),
        more_able_and_talented: Set(mat),
        english_as_additional_language: Set([(true, 1), (false, 7)]
            .choose_weighted(rng, |ch| ch.1)
            .unwrap()
            .0),
        free_school_meals: Set([(true, 1), (false, 5)]
            .choose_weighted(rng, |ch| ch.1)
            .unwrap()
            .0),
        additional_learning_needs: Set(aln),
        looked_after_child: Set([(true, 1), (false, 8)]
            .choose_weighted(rng, |ch| ch.1)
            .unwrap()
            .0),
